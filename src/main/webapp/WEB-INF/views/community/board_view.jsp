...
<head>
	...
	<!--  Reply CSS -->
	<!--  <link href ="../../css/replyview.css" rel="stylesheet"> -->
...
<body class="body-wrapper">
				...
				<div class="col-md-10 offset-md-1 col-lg-9 offset-lg-0">
					<article class="single-post">
						...
						<ul class="list-inline">
							<li class="list-inline-item">by <a href="">${board.nick}</a></li>
						...
						<!--  좋아요 / 댓글 버튼 part -->						
							...
							<li class="list-inline-item text-right">
								<img src="../../images/community/writing.png" style="width: auto; margin-bottom:0px;">
								<span id="count_reply" style="font-size:20px;margin-left:5px;">${totalCount}</span>
							</li>
						</ul>
						...
						<div class="block comment" style="margin-top: 10px;">
							...
							<c:choose>
								...
								<c:otherwise>
									<button type="button" class="btn btn-transparent" onclick="WriteReply('${board.pseq}');">댓글등록</button>
								</c:otherwise>
							</c:choose>
						</div>
					</article>
					...
					<form action="/community/deleteOk.do" method="post" name="dfrm">
						<input type="hidden" name="pseq" value="${board.pseq}" />
						<input type="hidden" name="boardType" value="${board.type}" />
							<div class="container">
									<input type="button" value="목록" class="btn btn-transparent" style="float: left;" onclick="goList('${board.type}');" />
	...
	const ReplyList = function(bno) {
		$.ajax({
			url : '/community/camp_replyList.do',
			type : 'get',
			data: {
				bno : bno
			},
			success : function(data) {
					...
					if(content == "") {	
						...
						if( depth == 0 ) {	
							...
							if("${nick}" != "") {
								listHtml += "&nbsp;&nbsp;";
								listHtml += "		<span>";
								listHtml += "			<a class='btn_rereply' data-toggle='collapse' href='#re_reply"+ rno +"' aria-expanded='false' aria-controls='collapseExample'>답글쓰기</a>";
								listHtml += "		</span>";
									
							}
						}else {	 // 답글일 때
							// row rereply 줄까말까 고민중
							listHtml += "	<div style='margin-left:50px;' class='col-10 rereply-content" + rno + "'>";	// div 4-2-3
							listHtml += "		<h6>";
							listHtml += "			<b>" + nick + "</b>";
							listHtml += "		</h6>";
						...
						if("${nick}" != ""){
							...
							if("${nick}" == nick ){
								...
								listHtml += "				<button style='color:orange;border:none;background-color:#FFF;' type='button' class='btn_rdelete' rno='"+ rno +"' depth='" + depth + "' bno='" + bno + "' grp='" + grp + "' data-bs-toggle='modal' data-bs-target='.bs-rdelete-modal-sm" + rno + "'>삭제</button>";
								listHtml += "			</span>";
								...
								listHtml += "				<button style='color:orange;border:none;background-color:#FFF;' type='button' class='btn btn--blue-2 btn-rdelete-modal' rno='"+ rno +"' depth='"+ depth +"' bno='"+bno+"' grp='"+grp+"'>";
								listHtml +=	"					삭제</button>";
								listHtml += "			</div>";
								listHtml +=	"		</div>";
								listHtml += "		</div>";
						...
						listHtml += "					<input style='width:70%;display:inline-block;' type='text' class='form-control' id='rereply" +rno + "'  placeholder='답글 입력...'>";
						//listHtml += "					<div class='row'>";
						//listHtml += "						<div class='col-11'></div>";
						//listHtml += "						<div id='btn_comment' class='col-1 btn_rereply'>";
						listHtml += "							<button style='width:130px;margin-left:10px;' type='button' class='btn btn-transparent write_rereply' rno='" + rno + "' bno='" + bno + "'>답글등록</button>";
						//listHtml += "						</div>";
						//listHtml += "					</div>";
						listHtml += "				</td>";
						listHtml += "			</tr>";
				...
				$('.btn_rdelete').on('click', function(){
					...
					//대댓글삭제	
						...
						DeleteReReply( $(this).attr('rno'), $(this).attr('bno'));
	...
	const WriteReReply = function(bno, rno) {
		...
		if(content == ""){
			...
			$.ajax({
				url: '/community/camp_write_rereply.do',
				type: 'POST',
				data: {
					rno : rno,
					bno : bno,
	...
	const DeleteReply = function(rno, bno){
		...
		$.ajax({
			url : '/community/camp_delete_reply.do',
			type : 'POST',
			data : {
				rno: rno,
				bno: bno
	...
	const DeleteReReply = function(rno, bno, grp){
		$.ajax({
			url : '/community/camp_delete_rereply.do',
			type : 'POST',
			data : {
				rno: rno,
				bno: bno
	...
	function goList(boardType) {
		var url = "l" == boardType ? "/community/camplog.do" : "t" == boardType ? "/community/camptip.do" : "/community/campgo.do";
		location.href=url;
	}
	
	$(document).ready(function(){
		ReplyList(${board.pseq});
	});
	</script>
	<!-- Footer part -->
